...
/*   Updated: 2018/05/06 16:26:32 by kyork            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			md5_write(t_md5state *st, t_u8 *buf, size_t len)
{
	...
	if (st->bufsz)
	{
		tmpsz = MIN((size_t)(MD5_BLOCK_SIZE - st->bufsz), len);
		ft_memcpy(st->buf + st->bufsz, buf, tmpsz);
		st->bufsz += tmpsz;
		ADJ_SZ(buf, len, st->len, tmpsz);
		if (st->bufsz == MD5_BLOCK_SIZE)
			md5_block(st, st->buf);
		if (st->bufsz == MD5_BLOCK_SIZE)
			st->bufsz = 0;
	}
	...
	{
		md5_block(st, buf);
		ADJ_SZ(buf, len, st->len, MD5_BLOCK_SIZE);
	}
...
t_md5state		md5_init(void)
{
	...
	st.s[2] = 0x98BADCFE;
	st.s[3] = 0x10325476;
	ft_bzero(st.buf, MD5_BLOCK_SIZE);
	st.bufsz = 0;
	st.len = 0;
	return (st);
}
